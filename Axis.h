/****************************************************************************
// $Archive: /ClearPath SC/Installer/Program Files/Teknic/ClearView/sdk/examples/6-Example-SingleThreaded(Polling)/Axis.h $
...
public:
	...
	void PrintStats(bool onInit = false) {
		...
		if (onInit) {
			printf("  [%2d]:\t\t**at startup**\t\t%lld\n",
				m_node->Info.Ex.Addr(), GetPosition());
		}
		else {
			printf("  [%2d]:\t\t%8d\t\t%lld\n",
