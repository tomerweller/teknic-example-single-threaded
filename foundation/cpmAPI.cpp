...
// $Revision: 97 $ $Date: 12/21/16 1:49p $
// $Workfile: cpmAPI.cpp $
//
// DESCRIPTION:
/**
...
// Firmware milestones
#define FW_MILESTONE_HAS_REGEN 0x5000
...
//	static const int rasFracLSD = 100;				// 0.01 is least significant digit
	...
	if (valIsBits) {
		...
		if (isEnhanced) {
			...
			#if 0
			subRate = (unsigned)codeParser.Fld.Code/256;
			selCode = (unsigned)codeParser.Fld.Code&0xFF;
			// Classic Style
			if (selCode < 16) {
				// Off end of table?
				if (selCode>sizeof(classicRASlen)/sizeof(int))
					return(ISC_RAS_OFF);
				if (subRate==0) {
					switch((int)convVal)  {
						case ISC_RAS_OFF: 
						...
						case ISC_RAS_44MS:
							return(44);
						default:
							return(ISC_RAS_OFF);			// Off end of table
					}
				}
				else if (subRate<7) {
					// Find and return the UI code from the bit pattern
					return((unsigned)(0.001*sampleTimeMicroSec
									  *(classicRASlen[selCode]+7)*(1<<(subRate-1))));
				} 
				else {
					// Illegal sub-rate
					return(ISC_RAS_OFF);
			...
			// Enhanced RAS?
			else if (selCode<=(deepCodeStart+enhancedRASlen)) {
				// Lookup the enhanced code stripping RAS selector off.
				return(iscFindRASuiCode(codeParser.Fld.Code));
			}
			// Off end of table
			else {
				return(ISC_RAS_OFF);
		...
		if (isEnhanced) {
			...
			#if 0
			// Which group, enhanced or classic?
			Uint32 lsDigs = (Uint32)(convVal*rasFracLSD+0.5);
			lsDigs %= 10;
			switch(lsDigs) {
				case 0:
					// Use classic selections for 44msec and below
					if (convVal >= 1.5 && convVal < 4)
						...
						return(ISC_RAS_24MS);
					else if (convVal >= 34 && convVal < 45)
						return(ISC_RAS_44MS);
					else {
						// Pick a good sub-sample length
						return(iscPickSubSampleRAS(convVal, sampleTimeMicroSec));
					}
					break;
				// Lookup in the cable
				default:
					// See if we have a match in our table and return the code
					for (int i=0; i<sizeof(eRASlookupTbl)/sizeof(eRASlookups); i++) {
						// Are we close to one in the table?
						if (fabs(convVal-eRASlookupTbl[i].requestCode)<0.005)
							return(eRASlookupTbl[i].nodeBits);
					}
					// Didn't find an exact match, try a close on
					eRASlookups *pSorted, *pEnd;
					pEnd = pSorted = (eRASlookups *)malloc(sizeof(eRASlookupTbl));
					// Copy items from proper table to RAM
					for (int i=0; i<sizeof(eRASlookupTbl)/sizeof(eRASlookups); i++) {
						bool isDeep = (eRASlookupTbl[i].nodeBits & 0xff) 
									>= deepCodeStart;
						if (isDeep && lsDigs==2 || !isDeep && lsDigs==1) {
							*pEnd++ = eRASlookupTbl[i];
						}
					}
					// Convert to list of items sorted by RAS length
					size_t nItems = (pEnd-pSorted);
					qsort(pSorted, nItems, sizeof(eRASlookups), sortRAS);
					pEnd = pSorted;
					// Find the closest on in the table and use it
					double lowVal, highVal;
					unsigned lowBits, highBits;
					for (size_t i=0; i<nItems-2; i++) {
						// Are we close to one in the table?
						lowVal = pSorted[i].requestCode;
						lowBits = pSorted[i].nodeBits;
						highVal = pSorted[i+1].requestCode;
						highBits = pSorted[i+1].nodeBits;
						if (convVal>=lowVal && convVal<highVal) {
							free(pSorted);
							return(lowBits);
						}
					}
...
/* 51 */ { 0, 0, PT_NONE,	NO_UNIT,		0, 1,		PARAM_NULL,				STR_UNKNOWN}, 	
...
/* 88 */ { 0, 1, PT_NONE, 	NO_UNIT,		4, 1,		PARAM_NULL,				STR_UNKNOWN}, 	
...
/* 91 */ { 0, 1, PT_CFG, 	DX_TICK,		2, 1,		PARAM_IN_RANGE_WIN,		STR_PARAM_POS_TRK_RNG},
...
/* 103*/ { 0, 0, PT_NONE, 	BIT_FIELD,		4, 1,		PARAM_NULL,				STR_UNKNOWN}, 
...
/* 270*/ { 0, 2, PT_MTR, 	NO_UNIT,		4, 1,		PARAM_RMS_LIM,			STR_DRV_RMS_LIM,		convertRMSlimit32}, 	
/* 271*/ { 0, 2, PT_MTR, 	TIME_MSEC,		2, 1,		PARAM_RMS_TC,			STR_DRV_RMS_TC,			convertRMStc}, 	
/* 272*/ { 0, 2, PT_CFG, 	DX_TICK,		2, 1,		PARAM_TRK_ERR_LIM,		STR_DRV_TRK_ERR_LIM}, 
...
/* 275*/ { 0, 0, PT_NONE, 	NO_UNIT,		0, 1,		PARAM_NULL,				STR_UNKNOWN}, 	
/* 276*/ { 0, 0, PT_NONE, 	NO_UNIT,		0, 1,		PARAM_NULL,				STR_UNKNOWN}, 	
...
/* 291*/ { 0, 1, PT_RO_RT, 	DEG_C,			2, 1,		PARAM_AMBIENT_TEMP,		STR_P_PWBA_TEMP}, 
...
/* 332*/ { 0, 0, PT_NONE, 	NO_UNIT,		0, 1,		PARAM_NULL,				STR_UNKNOWN}, 
...
/* 362*/ { 0, 2, PT_NV_RW, 	VOLT,			2, 1<<5,	PARAM_NULL,				STR_UNKNOWN}, 
/* 363*/ { 0, 2, PT_CFG, 	CURRENT,		2, 1<<15,	PARAM_HOMING_TRQ_LIM,	STR_DRV_HS_TRQ_LIM,		convertAmperes}, 
/* 364*/ { 0, 2, PT_NV_RW, 	NO_UNIT,		2, 1,		PARAM_NULL,				STR_UNKNOWN}, 	
...
/* 369*/ { 0, 2, PT_NV_RW, 	CURRENT,		2, 1<<15,	PARAM_REGEN_DUMP_RATE,	STR_UNKNOWN,			convertAmperes}, 	
/* 370*/ { 0, 2, PT_NV_RW,	TIME_MSEC,		2, 1,		PARAM_REGEN_RAMP_TIME,	STR_UNKNOWN,			convertFilt99pctMilliseconds}, 	
/* 371*/ { 0, 2, PT_NV_RW,	VOLT,			2, 1<<5,	PARAM_REGEN_TRIP_V,		STR_UNKNOWN}, 	
/* 372*/ { 0, 2, PT_NV_RW,	NO_UNIT,		2, 1<<12,	PARAM_NULL,				STR_UNKNOWN}, 	
...
/* 379*/ { 0, 0, PT_RO,		DEG_C,			2, 1U<<6,	PARAM_NULL,				STR_UNKNOWN}, 	
/* 380*/ { 0, 0, PT_RAM_RT,	DEG_C,			2, 1U<<6,	PARAM_NULL,				STR_UNKNOWN}, 	
/* 381*/ { 0, 0, PT_RAM_RT,	DEG_C,			2, 1U<<6,	PARAM_NULL,				STR_UNKNOWN}, 	
/* 382*/ { 0, 0, PT_RO,		DEG_C,			2, 1U<<6,	PARAM_NULL,				STR_UNKNOWN}, 	
/* 383*/ { 0, 0, PT_RO,		DEG_C,			2, 1U<<6,	PARAM_NULL,				STR_UNKNOWN}, 	
};
...
/* 517*/ { 0, 0, PT_NONE, 	NO_UNIT,		0, 1,		PARAM_NULL,				STR_UNKNOWN}, 	
/* 518*/ { 0, 0, PT_NONE, 	NO_UNIT,		0, 1,		PARAM_NULL,				STR_UNKNOWN}, 	
/* 519*/ { 0, 0, PT_NONE, 	NO_UNIT,		0, 1,		PARAM_NULL,				STR_UNKNOWN}, 	
...
/* 523*/ { 0, 0, PT_NONE, 	NO_UNIT,		0, 1,		PARAM_NULL,				STR_UNKNOWN}, 
...
/* 572*/ { 0, 0, PT_RO_RT,	NO_UNIT,		4, 1,		PARAM_NULL,					STR_UNKNOWN },
};
#undef UDSZ
#define PARAM_APP_COUNT (sizeof(cpmAppInfoDB)/sizeof(paramInfoLcl))
// Helpful macro to convert units
#define RPM_TO_TICK_SEC(rpm) ((rpm/60.)*ENC_DENS_DFLT)
#define RPM_TO_TICK_SEC2(rpm) ((rpm*ENC_DENS_DFLT)/60.)
...
#define DFLT_IMAX			20.						// Current scale maximum
...
#define ENC_DENS_DFLT		12800					// Ticks/turn
...
//    Type, Parameter,					    VAL,			FW Version
	...
	{ NVR,	CPM_P_HW_CONFIG_REG, 			DFLT_HW_CFG_REG }, 
	...
	{ NVR,	267,							DFLT_SENSORLESS_TRQ },	//SENSORLESS_TRQ (A)
...
*/
	...
	{ NVR,	CPM_P_DRV_TRQ_LIM,				DFLT_IMAX },			// A
	...
	{ NVR,	CPM_P_DRV_HARDSTOP_FLDBK_TRQ,	DFLT_IMAX },
	...
	{ NVR,  CPM_P_DRV_HARDSTOP_TRQ_LIM,		DFLT_IMAX * 0.2 },
	...
	{ NVR,  CPM_P_DRV_REGEN_LIM,			0,		FW_MILESTONE_HAS_REGEN },
};
			...
			iscStimState *pNewState)
{
	...
	switch(pNewState->mode)  {
		...
		case STIM_MOVE_REPEAT:
			...
			lVal = (nodelong)pNewState->amplitude;
			// Saturate @ largest value
			if (pNewState->amplitude > (double)LONG_MAX) lVal = (int32)LONG_MAX;
			...
			packetbuf *pNewValue)		// New value
{
	...
	theErr = netSetParameterEx
		(theMultiAddr, theParam, pNewValue, TRUE);
	return(theErr);
}
//																               *
//******************************************************************************
			...
			multiaddr theMultiAddr)			// Destination node
{
	...
	for (i=0 ; i<maxItems ; i++) {
		...
		if (inThisVers && advChkOK && fwOptOK) {  
