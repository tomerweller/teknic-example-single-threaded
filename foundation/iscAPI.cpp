...
// $Revision: 156 $ $Date: 12/22/16 12:11 $
// $Workfile: iscAPI.cpp $
//
// DESCRIPTION:
/**
...
#if 0			// TO_KILL DS 10/25/10
//*****************************************************************************
//	NAME																	  *
//		iscFindRASuiCode
//
//	DESCRIPTION:
///		Select the best sub-sampled RAS to achieve the target. If not in
///		table return the RAS off code.
///
/// 	\param xxx description
///		\return description
/// 
...
//
//	SYNOPSIS:
double iscFindRASuiCode(unsigned nodeBits) {
	// Search the valid RAS table for this bit pattern and return the
	// UI code for it.
	unsigned low,high,mid;

	low = 0;
	high = sizeof(eRASlookupTbl)/sizeof(eRASlookups) - 1;
	while (low <= high) {
	   mid = (low + high) / 2;
	   if (eRASlookupTbl[mid].nodeBits > nodeBits)
	       high = mid - 1;
	   else if (eRASlookupTbl[mid].nodeBits < nodeBits)
	       low = mid + 1;
	   else
	       return (eRASlookupTbl[mid].requestCode);
	}
	return(ISC_RAS_OFF);
}
//																			  *
...
//
//	DESCRIPTION:
///		Select the best sub-sampled RAS to achieve the target.
///
/// 	\param xxx description
///		\return description
/// 
...
//
//	SYNOPSIS:
double iscPickSubSampleRAS(double targetMS, double sampleMicroSecs)
{
	static const rasTarg rasTargs[] = {
		{44.000,   ISC_RAS_44MS},	// Prior classic boundary
		{47.365,   1026},  {48.549,   517},  {63.943,   772},  {66.311,   1281},
		{75.784,   1027},  {87.626,   518},  {94.731,   1282}, {97.099,   773},
		{127.886,  1028},  {132.623,  1537}, {151.569,  1283}, {175.252,  774},
		{189.461,  1538},  {194.198,  1029}, {255.773,  1284}, {303.138,  1539},
		{350.503,  1030},  {388.396,  1285}, {511.545,  1540}, {701.007,  1286},
		{776.791,  1541},  {1402.013, 1542}};
	// Find closest match in table
	for (int i=0; i<sizeof(rasTargs)/sizeof(rasTarg)-2; i++) {
		double lowVal, highVal;
		// Get midpoints between table items
		lowVal = (rasTargs[i].target+rasTargs[i+1].target)/2;
		highVal= (rasTargs[i+1].target+rasTargs[i+2].target)/2;
		// Stop if in range and return our center code.
		if (targetMS>lowVal && targetMS<=highVal) 
			return(rasTargs[i+1].code);
	}
	// Must be bigger than table, return last element
	return(rasTargs[sizeof(rasTargs)/sizeof(rasTarg)-1].code);
}
//																			  *
//*****************************************************************************


// qsort comparison function to convert from bit oriented to request oriented
// sort order.
int (__cdecl sortRAS )(const void *pElement1, const void *pElement2)  {
	register eRASlookups *p1 = (eRASlookups *)pElement1;
	register eRASlookups *p2 = (eRASlookups *)pElement2;
	return((int)(p1->requestCode*100 - p2->requestCode*100));
}
#endif
