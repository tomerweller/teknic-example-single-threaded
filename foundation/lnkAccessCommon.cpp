...
#define ENTER_LOCK(where) \
	_RPT1(_CRT_WARN, "enterLock	%s\n", where); pNCS->enterCmdLock();
#define EXIT_LOCK(where) \
	_RPT1(_CRT_WARN, "exitLock	%s\n", where); pNCS->exitCmdLock();
#else
...
#define TRACE_ERR_HNDL			T_ON	// Print error handler actions
...
const char *pwrTypes[PWR_CNT] = {
	"Unknown", "DC", "#2", "#3", "3-phase (fwd)", "3-phase (rev)", "#6", "#7", 
	"Single (X-Y)", "Single (X-Z)", "Single (Y-Z)", "#12", "#13", "#14", "#15"
};
/// \endcond																   *
//******************************************************************************
	...
	double timeStamp)
{
	...
	if (theErr != MN_OK && pNCS->cNum == 1) {
		_RPT4(_CRT_WARN, "%.1f RX Error(%d): 0x%x, thread=" THREAD_RADIX "\n",
					...
					if (readBuf.Byte.BufferSize == 0) {
						...
						if (readBuf.Fld.Src == MN_SRC_HOST) {
							...
							if (isNetReject) {
								...
								#if TRACE_NET_ERR
								_RPT1(_CRT_WARN, "readThread(%d) host detected err: \n", pNCS->cNum);
	...
	netaddr cNum)
{
	...
	#endif
	// Exar/Windows appears to get messed up Break detection, we will close and open
	// port to clear out issues.
	//	pNCS->pSerialPort->Close();
	//	if (pNCS->pSerialPort->OpenComPort(SysInventory[cNum].PhysPortSpecifier.PortNumber)) {
	//		return(cnErrCode(MN_ERR_INIT_FAILED_BASE+cNum));
	//	}
	...
	serErr = (pNCS->pSerialPort->Setup(MN_NET_BAUD_RATE,
		...
		(rtsState ? CSerial::ERTSSet
			: CSerial::ERTSClear)));
	if (serErr) {
		if (serErr == CSerial::API_ERROR_PORT_UNAVAILABLE
			|| serErr == CSerial::API_ERROR_INVALID_HANDLE) {
			_RPT3(_CRT_WARN, "%.1f infcResetNetRate(%d) port unavailable! Err=0x%x\n",
				infcCoreTime(), cNum, serErr);
			#if 0
			{   // TO_KILL
				WCHAR foo[100];
				wsprintf(foo, _T("resetNet: port setup failed 0x%x\n"), serErr);
				OutputDebugStr(foo);
			}
			#endif
			return(MN_ERR_PORT_PROBLEM);
	...
	for (int i = 0; i<5; i++) {
		...
		// 1/3/2011 JS: Increased sleep time for 16-node network
		//infcSleep(Uint32(MN_BREAK_DURATION_MS*2*2));
		...
		if (currentErrors.BREAKcnt == 0) {
			...
			{
				WCHAR foo[100];	   // TO_KILL
				wsprintf(foo, _T("No break detected\n"));
				OutputDebugStr(foo);
			}
			#endif
	...
	netRates newRate)
{
	...
	#endif
	// Save our new rate in case we have to revert
	//oldRate = SysInventory[cNum].PhysPortSpecifier.PortRate;
	...
	// All code past here should exit through exit_loc 
	// to properly cleanup.
	...
	for (int i = 0; i<3; i++) {
		...
		_RPT3(_CRT_WARN, "%.1f in infcSetNetRate(%d) calling infcResetNetRate,"
			"thread ID=" THREAD_RADIX "\n",
			infcCoreTime(), cNum, nodeulong(infcThreadID()));
		#endif
		theErr = infcResetNetRate(cNum);
		if (theErr != MN_OK)
			continue;
		...
		if (theErr == MN_OK) {
			#if TRACE_BAUD
			_RPT3(_CRT_WARN, "%.1f infcSetNetRate(%d) nodes now @ %d\n",
				infcCoreTime(), cNum, newRate);
			#endif
			// Setup/restore our port to user's settings
			bool dtrState = pNCS->pSerialPort->GetDTR();
			bool rtsState = pNCS->pSerialPort->GetRTS();
			if (pNCS->pSerialPort->Setup(newRate,
				...
				infcErrInfo errInfo;
				errInfo.cNum = cNum;
				errInfo.errCode = MN_ERR_BAUD_NOT_SUPPORTED;
				errInfo.node = -1;
				infcFireErrCallback(&errInfo);
				_RPT3(_CRT_WARN, "%.1f infcSetNetRate(%d) port doesn't support %d\n",
					infcCoreTime(), cNum, newRate);
				#if TRACE_BREAK
				_RPT3(_CRT_WARN, "%.1f in infcSetNetRate(%d) failure calling infcResetNetRate,"
					"thread ID=" THREAD_RADIX "\n",
					infcCoreTime(), cNum, nodeulong(infcThreadID()));
				#endif
				// Attempt to restore default baud rate, ignoring outcome.
				infcResetNetRate(cNum);
				theErr = MN_ERR_BAUD_NOT_SUPPORTED;
				goto exit_loc;
			}
			#if TRACE_BAUD
			_RPT3(_CRT_WARN, "%.1f infcSetNetRate(%d) port now @ %d\n",
				infcCoreTime(), cNum, newRate);
			#endif
			// Unstack recovery and error processing
		...
		// We failed recover
		_RPT4(_CRT_WARN, "%.1f infcSetNetRate(%d), attempt #%d failed=0x%x\n",
			infcCoreTime(), cNum, i, theErr);
	}
exit_loc:
	// - - - - - - - - - - - - - - - - - - - - - - - - - - 
	...
	netaddr cNum)					// cNum = the COM Port to be opened
{
	...
	if (theErr == MN_OK) {
		...
		winErr = pNCS->pSerialPort->Setup(MN_NET_BAUD_RATE,
			...
			: CSerial::ERTSClear);
		if (winErr) {
			// Assume baud is illegal for some reason
			return(MN_ERR_BAUD_NOT_SUPPORTED);
		}
...

{
	...
	_RPT3(_CRT_WARN, "%.1f infcStopController %d: stopped code=%x\n",
		infcCoreTime(), cNum, errRet);
	#endif
	return ((cnErrCode)errRet);
}
//																			  *
//*****************************************************************************
...
//
//	DESCRIPTION:
//		Send a serial character via the EPP port in Serial Mode. This will wait
//		for up to 1 second before declaring failure.
//
//	RETURNS:
//		error code
//
//	SYNOPSIS:
MN_EXPORT cnErrCode MN_DECL infcSendChar(
	netaddr cNum,
	nodeulong theChar)
{
	BOOL errRet;
	...
	if (pNCS && !pNCS->pSerialPort)
		return(MN_ERR_CLOSED);
	//timeOut = (ULONG)infcCoreTime() + 1000UL;
	// Wait for character or timeout to occur
	//while (infcCoreTime() < timeOut && !infcSendCharAvailable(cNum))
	//	;
	// We can send, do it
	errRet = pNCS->pSerialPort->SendChars((char*)&theChar, 1);
	return(errRet ? MN_OK : MN_ERR_TX_BUSY);
}
//																			  *
//*****************************************************************************
...
//
//	SYNOPSIS:
MN_EXPORT cnErrCode MN_DECL infcSendChars(
	netaddr cNum,
	nodelong nChars,
	nodeuchar theChar[])
{
	...
	//	;
	// We can send, do it
	errRet = pNCS->pSerialPort->SendChars((char*)theChar, nChars);
	return(errRet ? MN_OK : MN_ERR_TX_BUSY);
}
//																			  *
//*****************************************************************************
...
//
//	DESCRIPTION:
//		Get the next available character from the serial interface. This function
//		will return an error if no character is available.
//
//	RETURNS:
//		#cnErrCode
//
//	SYNOPSIS:
MN_EXPORT cnErrCode MN_DECL infcGetChar(
	netaddr cNum,
	nodeulong *portChar)
{
	BOOL errRet;
	...
	if (!portChar)
		return(MN_ERR_BADARG);
	// If we have a port, get the next character from the buffer
	if (pNCS && pNCS->pSerialPort) {
		errRet = pNCS->pSerialPort->GetChar((char*)portChar);
		// Insure MSBs are cleared
		*portChar &= 0xff;
		// Return MN_OK if we have it
		return(errRet ? MN_OK : MN_ERR_VALUE);
	}
	// Nothing to return
	return(MN_ERR_PORT_PROBLEM);
}
...
//*****************************************************************************
// NAME																          *
//		infcGetCharAvailable
//
//	DESCRIPTION:
//		Returns the availablity of a serial character via the infcGetChar
//		function. This makes sure that at least two in a row are the same.
//
//	RETURNS:
//		TRUE if a character is available
//
//	SYNOPSIS:
MN_EXPORT nodelong MN_DECL infcGetCharAvailable(
	netaddr cNum)
{
	netStateInfo *pNCS = SysInventory[cNum].pNCS;
	if (pNCS && pNCS->pSerialPort)
		return pNCS->pSerialPort->GetCharsAvailable();
	return(0);
}
//																			 *
//****************************************************************************
...
/*****************************************************************************
*	NAME
*		infcSendCharAvailable
*
*	DESCRIPTION:
*		Returns whether the infcSendChar function can accept another character
*		for transmission.
*
*	RETURNS:
...
*
*	SYNOPSIS: 															    */
MN_EXPORT nodebool MN_DECL infcSendCharAvailable(
	netaddr cNum)
{
	netStateInfo *pNCS = SysInventory[cNum].pNCS;
	if (pNCS && pNCS->pSerialPort)
		return pNCS->pSerialPort->Send_Available();
	return(FALSE);
}
//																			 *
//****************************************************************************
...
//
//	DESCRIPTION:
//		Put the interface into serial port mode.
//
//	RETURNS:
//		MN_OK when in the mode, else a standard error code
//
//	SYNOPSIS:
MN_EXPORT cnErrCode MN_DECL infcEnterSerialMode(
	netaddr cNum,
	mnSerialSettings *pMode)
{
	cnErrCode theErr = MN_OK;

	netStateInfo *pNCS = SysInventory[cNum].pNCS;
	#ifdef _DEBUG
	double startTime = infcCoreTime();
	#endif
	#if TRACE_HIGH_LEVEL||TRACE_SERIAL
	_RPT1(_CRT_WARN, "%.1f infcEnterSerialMode: entered\n", infcCoreTime());
	#endif
	// Knock offline
	infcFireNetEvent(cNum, NODES_OFFLINE);

	// Protect from crash if we are already shutdown
	if (!pNCS || !pNCS->pSerialPort)
		return(MN_ERR_CLOSED);
	// Turn off the poller
	pNCS->pPollerThread->Stop();
	infcHaltAutoNetDiscovery(cNum);
	// Stop the read thread from interfering
	pNCS->ReadThread.Stop();
	// Put us in serial mode
	pNCS->pSerialPort->SetMode(SERIALMODE_SERIAL);
	nodelong portSpeed;
	// Coast the speed if not specifie.  (past behavior).
	if (pMode->bitSpeed)
		portSpeed = pMode->bitSpeed;
	else
		portSpeed = pNCS->pSerialPort->GetBaudrate();
	pNCS->pSerialPort->Setup(portSpeed, CSerial::EData8, CSerial::EParNone,
		CSerial::EStop1, CSerial::EDTRClear, CSerial::ERTSClear);
	SysInventory[cNum].OpenStateNext(FLASHING);

	_RPT3(_CRT_WARN, "%.1f infcEnterSerialMode: exec time %f, err=0x%x\n",
		infcCoreTime(), infcCoreTime() - startTime,
		theErr);
	infcSleep(100);
	// Signal readyness here
	infcFireNetEvent(cNum, NODES_FLASH_MODE);
...
*
*	SYNOPSIS: 																*/
MN_EXPORT cnErrCode MN_DECL infcExitSerialMode(
	netaddr cNum)
{
	#if TRACE_HIGH_LEVEL||TRACE_SERIAL
	_RPT1(_CRT_WARN, "%.1f infcExitSerialMode: entered\n", infcCoreTime());
	#endif
	netStateInfo *pNCS = SysInventory[cNum].pNCS;
	if (!pNCS || !pNCS->pSerialPort)
		return(MN_ERR_CLOSED);
	...
	// Let junk pile up
	infcSleep(100);
	// Kill anything waiting
	pNCS->pSerialPort->Flush();
	// Back to packet mode
	pNCS->pSerialPort->SetMode(SERIALMODE_MERIDIAN_7BIT_PACKET);
	#if TRACE_HIGH_LEVEL||TRACE_SERIAL
	_RPT1(_CRT_WARN, "%.1f infcExitSerialMode: exited\n", infcCoreTime());
	#endif
	// Clear any application's idea we are in FLASH mode.
	infcFireNetEvent(cNum, NODES_OFFLINE);
	return MN_OK;
}
//																			 *
	...
	if (initializing) {
		...
		if (theNet.Initializing == 0) {
			...
			case MN_ERR_TEST_INCOMPLETE:
				// Ignore errors at this point (??04-30-2012)
				if (theNet.PhysPortSpecifier.PortRate
					!= theNet.pNCS->pSerialPort->GetBaudrate()) {
					#if defined(_DEBUG) && ((TRACE_CALLBACK&&TRACE_LOW_LEVEL) || LCL_INIT_PR)
					cnErrCode errSpd;
					errSpd =
						#endif
						infcSetNetRate(cNum,
							theNet.PhysPortSpecifier.PortRate);
					#if defined(_DEBUG) && ((TRACE_CALLBACK&&TRACE_LOW_LEVEL) || LCL_INIT_PR)
					_RPT4(_CRT_WARN, "%.1f infcSetInitializeMode(%d): restoring baud, rate=%d, err=0x%x\n",
						infcCoreTime(), cNum, theNet.PhysPortSpecifier.PortRate, errSpd);
					#endif
				}
				break;
			default:
	...
	netStateInfo *pNCS, packetbuf &readBuf)
{
	...
	case MN_PKT_TYPE_EXTEND_HIGH:
		...
		case MN_CTL_EXT_NET_DIAG_INFO:
			...
			// Copy to our state
			*pLog = *pStats;
			_RPT2(_CRT_WARN, "readThread(%d) node %d reported err: \n",
	...
	case MN_PKT_TYPE_EXTEND_LOW:
		...
		case MN_CTL_EXT_DATA_ACQ:	// Data return originated from the node and is a dataAcq point
			...
			if (pNCS->DataAcqInit[respAddr]
				...
				_RPT2(_CRT_WARN, "%.1f data acq seq err node %d\n", infcCoreTime(), respAddr);
				infcFireErrCallback(&errInfo);
				#if TRACE_DACQ_SEQ
...
void netPollerThread::Start()
{
	...
	#if TRACE_POLL_THRD||TRACE_THREAD
	_RPT2(_CRT_WARN, "%.1f netPollerThread(%d): starting poll...\n",
		infcCoreTime(), pNCS->cNum);
	#endif
	// Lock down the read state
	m_critSection.Lock();
	...
	if (SysInventory[pNCS->cNum].NodeInfo[0].theID.fld.devType == NODEID_MD) {
		errRet = netGetParameter(MULTI_ADDR(pNCS->cNum, 0), ISC_P_FW_VERSION, &fwVers);
		if (errRet == MN_OK && FWVER->verCode >= ISC_MILESTONE_POLLER) {
			pollerSupported = true;
		}
		}
	if (SysInventory[pNCS->cNum].NodeInfo[0].theID.fld.devType == NODEID_CS) {
...
{
	#if TRACE_POLL_THRD||TRACE_THREAD
	_RPT2(_CRT_WARN, "%.1f netPollerThread(%d): stopping poll...\n",
		infcCoreTime(), pNCS->cNum);
	#endif
	// Lock down the read state
	m_critSection.Lock();
