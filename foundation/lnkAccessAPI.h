...
// Provides a polling friendly raw character interface to the serial port.
// This needs to be VB friendly for the flash tools.
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 

// Serial port mode settings in VB friendly terms
typedef struct _mnSerialSettings {
	nodelong bitSpeed;		// Serial data
	nodelong fmt;			// Serial format
} mnSerialSettings;
		
// Serial Port Emulation API for Flash
...
MN_EXPORT nodebool MN_DECL infcSendCharAvailable(
		netaddr cNum);
// Put port in polled serial mode
MN_EXPORT cnErrCode MN_DECL infcEnterSerialMode(
		netaddr cNum,
		mnSerialSettings *mode);
// Get port out of polled serial mode
