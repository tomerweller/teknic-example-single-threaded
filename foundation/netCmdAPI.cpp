...
// $Revision: 225 $ $Date: 12/22/16 12:17 $
...
const char *CNFG_VCS_KEY = "VCS";
const char *CNFG_MOD_VER_KEY = "Writer";
const char *CNFG_SYS_VCS_KEY = "SysWriter";
// NOTE: the insure revision control does not change CNFG_SYS_VCS_TEMPLATE
const char *CNFG_SYS_VCS_TEMPLATE = "$" "Revision" "$ $" "Archive" "$ $" "Date$";
...
//
//	DESCRIPTION:
///		Enter node communications bypass mode. This turns a network drives into
///		a "wire" with the exception of the addressed node. This is typically
///		used for flashing this node.  The unaddressed nodes will forward all
///		received characters without any interpretation until netExitBypassMode
///		has been run or they have been power cycled back to their "Online Idle"
///		state.
///
/// 	\param multiAddr_nodeException Exception address, all other nodes will
///			enter the bypass mode.
///		\return MN_OK when the channel is setup for bypass mode.
///
/// 	Detailed description.
//
//	SYNOPSIS:
MN_EXPORT cnErrCode MN_DECL netEnterBypassMode(
		multiaddr multiAddr_nodeException)
{
	packetbuf theCmd, theResp;
	cnErrCode theErr;
	netaddr cNum;
	...
	_RPT2(_CRT_WARN, "%.1f netEnterBypassMode(%d): entered\n", 
			infcCoreTime(), multiAddr_nodeException);
#endif
	cNum = coreController(multiAddr_nodeException);
	// Prevent the background poller from interfering
	theErr = infcBackgroundPollControl(cNum, false);
	if (theErr != MN_OK)
		return(theErr);

#if TRACE_HIGH_LEVEL||TRACE_SERIAL
	_RPT1(_CRT_WARN, "%.1f netEnterBypassMode: sending cmd\n", infcCoreTime());
#endif
	theCmd.Fld.Src = MN_SRC_HOST;
	theCmd.Fld.PktType = MN_PKT_TYPE_EXTEND_HIGH;
	theCmd.Fld.Addr = NODE_ADDR(multiAddr_nodeException);
	theCmd.Fld.PktLen = 1;
	theCmd.Fld.Mode = theCmd.Fld.Zero1 = 0;	// Fill in unused with 0
	theCmd.Byte.Buffer[CMD_LOC] = MN_CTL_EXT_ENTER_MULTI_BYPASS;
	theCmd.Byte.BufferSize = theCmd.Fld.PktLen + MN_API_PACKET_HDR_LEN;

	// Run ignoring the response. This is done to let the threaded driver
	// internally flush after this command runs
	theErr = infcRunCommand(cNum, &theCmd, &theResp);
	// Run and wait for the response. This is done to let the threaded driver
	// internally flush after this command runs
	theErr = infcRunCommand(cNum, &theCmd, &theResp);
	// Insure we got our packet back!
	if (theErr == MN_OK
	&& (theResp.Byte.Buffer[0]&0x7f) == (theCmd.Byte.Buffer[0]&0x7f)
	&& theResp.Byte.Buffer[1] == theCmd.Byte.Buffer[1]) {
		infcFlush(cNum);
#if TRACE_HIGH_LEVEL||TRACE_SERIAL
		_RPT2(_CRT_WARN, "%.1f netEnterBypassMode: exec time = %.2f\n",
						 infcCoreTime(), infcCoreTime()-startTime);
#endif
		return(MN_OK);
	}
#if TRACE_HIGH_LEVEL||TRACE_SERIAL
	_RPT3(_CRT_WARN, "%.1f netEnterBypassMode: failed, exec time = %.2f, err=0x%x\n",
					 infcCoreTime(), infcCoreTime()-startTime, theErr);
#endif
	return(MN_ERR_RESP_FAILED);
}
//																			  *
//*****************************************************************************
...
//
//	DESCRIPTION:
///		This function will undo the netEnterBypass mode and restore the network
///		to the communications state prior to entry into bypass mode.
///
...
//
//	SYNOPSIS:
MN_EXPORT cnErrCode MN_DECL netExitBypassMode(
		netaddr cNum)
{
	cnErrCode theErr = MN_OK;
	portSpec thePortSpec;
#if TRACE_HIGH_LEVEL||TRACE_SERIAL
	double startTime = infcCoreTime();
	_RPT1(_CRT_WARN, "%.1f netExitBypassMode: entered\n", startTime);
#endif

	// Insure we are starting from idle
	//infcFlush(cNum);

	theErr = infcGetPortSpecifier(cNum, &thePortSpec);
	if (theErr == MN_OK) {
		theErr = infcResetNetRate(cNum);
		if (theErr == MN_OK) {
			theErr = netEnumerate(cNum);
			if (theErr == MN_OK)
				theErr = infcSetNetRate(cNum, thePortSpec.PortRate);
		}
	}
	// Kill any "extras"
	//infcFlush(cNum);
#if TRACE_HIGH_LEVEL
	_RPT3(_CRT_WARN, "%.1f netExitBypassMode: exec time = %.2f, err=0x%x\n",
					 infcCoreTime(), infcCoreTime()-startTime, theErr);
#endif
	return(theErr);
}
...
//
//	DESCRIPTION:
//		Start the node boot loader for those nodes that support it.
//
//	RETURNS:
//		standard error code
//
//	SYNOPSIS:
MN_EXPORT cnErrCode MN_DECL netStartBootLoader(
		multiaddr theMultiAddr) 			// node address

{
...
#if TRACE_HIGH_LEVEL||TRACE_SERIAL
	double startTime = infcCoreTime();
	_RPT2(_CRT_WARN, "%.1f netStartBootLoader(%d): entered\n", 
		infcCoreTime(), theMultiAddr);
#endif
	cNum = coreController(theMultiAddr);
	// Keep the background poller from interfering.
	theErr = infcBackgroundPollControl(cNum, false);
	if (theErr != MN_OK)
		return(theErr);
#if TRACE_HIGH_LEVEL||TRACE_SERIAL
	_RPT2(_CRT_WARN, "%.1f netStartBootLoader(%d): sending command\n", 
		infcCoreTime(), theMultiAddr);
#endif
	theCmd.Fld.Src = MN_SRC_HOST;
	theCmd.Fld.PktType = MN_PKT_TYPE_CMD;
	...
	theCmd.Fld.Mode = theCmd.Fld.Zero1 = 0;	// Fill in unused with 0
	theCmd.Byte.Buffer[CMD_LOC] = MN_CMD_NET_ACCESS;
	theCmd.Byte.Buffer[CMD_LOC+1] = (nodechar)(MN_BOOT_START_CODE);
	theCmd.Byte.Buffer[CMD_LOC+2] = (nodechar)(MN_BOOT_START_CODE>>8);
	theCmd.Byte.Buffer[CMD_LOC+3] = (nodechar)(MN_BOOT_START_CODE>>16);
	theCmd.Byte.Buffer[CMD_LOC+4] = (nodechar)(MN_BOOT_START_CODE>>24);
	theCmd.Byte.BufferSize = theCmd.Fld.PktLen + MN_API_PACKET_HDR_LEN;

	// Run and wait for the response. This is done to let the threaded driver
	// internally flush after this command runs
	theErr = infcRunCommand(cNum, &theCmd, &theResp);
	if (theErr == MN_OK && theResp.Fld.PktType == MN_PKT_TYPE_RESP)
		return(MN_OK);
	return(MN_ERR_RESP_FAILED);
}
//																		      *
 ...
 *
 *	SYNOPSIS: 															    */
cnErrCode MN_DECL netGetParameterInfo(
		multiaddr theMultiAddr,			// Node address
		...
		const mnStatusReg *pStatusEventMask)
{
	...
	// Send them out
	theErr = netSetParameterEx(theMultiAddr, pNum, &paramVal, false);
	return theErr;
}
//																			  *
//*****************************************************************************
	...
	const char *theVal)
{
	...
	iniparser_set(d, itemKey, theVal);
	return MN_OK;
}
cnErrCode setConfigItem(
    dictionary *d,
	...
	const paramInfo &info)
{
	...
	int theLength = sscanf(iniItem, "%" SCAN_MAX "s", valStr);
	if (theLength==0) {
		...
		const char *pFilePath)
{
	...
	try {
		...
		// Write out source control template
		setConfigItem(d, sectName, CNFG_VCS_KEY, CNFG_SYS_VCS_TEMPLATE);
